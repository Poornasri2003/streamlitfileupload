...
def handle_upload(text):
    """Process the uploaded text and save it to a file."""
    # Save the uploaded text to a file
...
def functmain():
    try:
        ...
        with st.sidebar:
            st.title("Menu:")
            uploaded_file = st.file_uploader("Upload your Word doc", type=['docx'])

            if uploaded_file is not None:
                raw_text = get_word_text(uploaded_file.getvalue())
                st.write("File content:")
                st.text(raw_text)

                if st.button("Submit & Process"):
                    with st.spinner("Processing..."):
                        handle_upload(raw_text)
                    st.success("Done")
